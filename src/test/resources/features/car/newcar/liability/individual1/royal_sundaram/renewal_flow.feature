Feature : Car Insurance Renewal
  ...
  Scenario Outline : Personal tab
    ...
    And   User select the occupation
    ...
    And   User clicks on the next button
    Examples:
      | first_name | last_name | nominee_firstname | nominee_lastname | nominee_relationship | nominee_age |
  ...
  Scenario Outline: Vehicle tab
    ...
    And   User enters the puc certificate as "<puc_number>"
    And   User enters the puc certificate expiry date "<puc_date>"
    And   User clicks on the next button

    Examples:
      | policy_number | engine_number | chassis_number | puc_number | puc_date |
